...
if [ -f "$DDS_PROFILE" ]; then
    echo -e "${GREEN_BOLD}Base CycloneDDS profile: $(grep -m1 '<MaxMessageSize>' "$DDS_PROFILE" | xargs || true)${RESET}"
fi
...
docker run \
    ...
    -e PATH=/opt/ros/jazzy/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
